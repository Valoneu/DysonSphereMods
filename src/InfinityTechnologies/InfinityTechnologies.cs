...
namespace InfinityTechnologies
{
        ...
        public void Recalculate()
        {
            ...
            float GetLogBonus(int techId, float logFactor, float slowness = 1f)
            {
                if (history.techStates.TryGetValue(techId, out var state))
                {
                    ...
                    if (proto != null)
                    {
                        int deltaLevel = Math.Max(0, state.curLevel - proto.Level);
                        if (deltaLevel > 0)
                            return 1f + logFactor * (float)Math.Log10(1.0 + (double)deltaLevel / slowness);
                    }
                }
                return 1f;
            }
            WirelessPowerMultiplier = GetLevelBonus(9002, 0.2f);
            SpherePowerMultiplier = GetLogBonus(9051, 3.0f, 10f);
    ...
    public static class TechDefinitions
    {
        public static void Register()
        {
            ...
            RegisterInfinite(9051, "TechName_9051", "TechDesc_9051_Log", "Icons/Tech/1501", 106, 3.0, 10000000, 10000000, new Vector2(50, 8));
            ...
            RegisterInfinite(9055, "TechName_9055", "TechDesc_9055_Log", "Icons/ItemRecipe/6006", 110, 16.33, 5000000, 5000000, new Vector2(50, -4));
        }
